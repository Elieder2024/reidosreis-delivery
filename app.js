    ...
    return `
      <div style="background: #fffbe6; border: 1px solid var(--border-color); border-radius: var(--radius-md); padding: 1.25rem; margin-bottom: 1rem; box-shadow: var(--shadow-sm);">
        ...
        <div style="margin-top: 0.75rem; display: flex; justify-content: flex-end;">
          <button class="btn btn-gold btn-sm" onclick="openOrderReceiptModal('${o.id}')">
  ...
  container.innerHTML = `
    <div id="printable-receipt-content" style="background: #ffffff; border: 1px dashed #cbd5e1; padding: 1.25rem; border-radius: var(--radius-sm); font-family: 'Courier New', Courier, monospace; color: #0f172a;">
      <div style="text-align: center; border-bottom: 2px dashed #94a3b8; padding-bottom: 0.75rem; margin-bottom: 0.75rem;">
        <h3 style="font-size: 1.1rem; font-weight: 900; margin-bottom: 2px;">👑 REI DOS REIS DELIVERY</h3>
        <p style="font-size: 0.75rem;">Pizzaria Tradicional & Combos</p>
        <p style="font-size: 0.7rem;">Balneário Camboriú - SC • Tel/WA: (47) 98880-2254</p>
        <small style="display: block; font-style: italic; font-size: 0.68rem; margin-top: 4px;">"Louvado e engrandecido seja o nome do Senhor Jesus Cristo"</small>
      </div>

      <div style="font-size: 0.78rem; border-bottom: 1px dashed #cbd5e1; padding-bottom: 0.5rem; margin-bottom: 0.5rem;">
        <div><strong>PEDIDO Nº:</strong> <span style="font-weight: 900; color: #b45309;">${order.id}</span></div>
      ...
      <div style="font-size: 0.82rem; font-weight: 800; display: flex; justify-content: space-between; border-bottom: 2px dashed #94a3b8; padding-bottom: 0.5rem; margin-bottom: 0.75rem;">
        ...
        <span style="font-size: 1rem; color: #b45309;">TOTAL: R$ ${priceNum.toFixed(2).replace('.', ',')}</span>
      </div>
...
function sendReceiptWhatsApp() {
  ...
  const msg = `*👑 COMPROVANTE REI DOS REIS DELIVERY*%0A` +
    `*Pedido:* ${o.id}%0A` +
    `*Data:* ${o.date || 'Hoje'}%0A` +
    ...
    `_Obrigado pela preferência! Deus abençoe! 👑_`;

  const phone = o.clientPhone ? o.clientPhone.replace(/\D/g, '') : '5547988802254';
